...
// //     const res = await fetch('/api/chat', { // Replace with your actual route name
// //       method: 'POST',
// //       headers: {
...
// //         simulateTyping(data.error || "An unknown error occurred.");
// //         return;
// //     }
...
// //     simulateTyping("⚠️ Sorry, something went wrong. Try again.");
// //   }
// // };// New handleSubmit function
...
export default function GeminiDemo() {
  ...
  // Handle form submission (now fully client-side)
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!input.trim()) return
    ...
    try {
      ...
      simulateTyping("⚠️ Sorry, something went wrong. Please try again.")
    }
  }
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", preventScroll, { passive: false })
    setTimeout(() => window.removeEventListener("scroll", preventScroll), 1000)
  }, [])
        ...
        <div className="h-[400px] overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <h3 className="text-lg font-medium text-white/70 mb-2">Gemini AI Demo</h3>
              <p className="text-sm text-white/50 max-w-md">
                This is a real Gemini AI integration. Type a message to start chatting.
              </p>
            </div>
          ) : (
            messages.map((message, index) => (
